...
function sortBucketKeys(bucketKeys) {
  return [...bucketKeys].sort((left, right) => {
    const leftIndex = BUCKET_KEYS.indexOf(left);
    const rightIndex = BUCKET_KEYS.indexOf(right);
...
function buildRegistrySource({ fortunes, bucketKeys = BUCKET_KEYS }) {
  ...
  return {
    ...
    fortunes: fortunes.map((fortune) => ({
      ...
      alsoFits: Array.isArray(fortune.alsoFits)
        ? sortBucketKeys(fortune.alsoFits)
        : [],
...
function writeRegistry({ fortunes, bucketKeys }) {
  const source = buildRegistrySource({
    fortunes: fortunes.map((fortune) => ({
      ...
      text: fortune.text,
      primaryBucket: fortune.primaryBucket,
      alsoFits: sortBucketKeys(fortune.alsoFits || []),
      scope: fortune.scope === 'shared' ? 'shared' : 'specific',
    })),
    bucketKeys,
  });
...
function toEditorFortune(fortune) {
  return {
    ...fortune,
    alsoFits: [...(fortune.alsoFits || [])],
  };
}
