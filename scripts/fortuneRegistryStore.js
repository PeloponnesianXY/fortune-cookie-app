...
const REGISTRY_FILE_PATH = path.resolve(__dirname, '../data/fortunesRegistry.js');
...
const STORAGE_BUCKET_KEYS = CANONICAL_BUCKET_KEYS.map((bucket) => (
  bucket === 'unknown' ? 'weird' : bucket
));

function normalizeStorageBucketKey(bucket) {
  return bucket === 'unknown' ? 'weird' : bucket;
}
